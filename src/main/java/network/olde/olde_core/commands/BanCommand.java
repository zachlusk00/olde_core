...
public class BanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        String example = "Example: /ban <player> <reason>";
        if (commandSender instanceof Player) {
            Player player = (Player) commandSender;

            if (player.hasPermission("oldecore.ban")) {
                if (strings.length == 1) {
                    // /ban <player>
                    ...
                    if (target != null) {
                        ...
                        player.sendMessage( target.getDisplayName() + ChatColor.RED + "has been banned ");

                        BanList<?> banList = Bukkit.getServer().getBanList(BanList.Type.PROFILE);
                        banList.addBan(target.getDisplayName(),"Banned by " + player.getDisplayName(), null, player.getDisplayName());
                    }
                    else {
                    ...
                    if (target != null) {
                        ...
                        BanList<?> banList = Bukkit.getServer().getBanList(BanList.Type.PROFILE);
                        banList.addBan(target.getDisplayName(),ChatColor.RED + stringBuilder.toString(), null, player.getDisplayName());
                    }
                    else {
                        player.sendMessage(ChatColor.GOLD + "Player not found.");
                    }
        ...
        // Console
            ...
            if (strings.length == 1) {
                ...
                if (target != null) {
                    ...
                    BanList<?> banList = Bukkit.getServer().getBanList(BanList.Type.PROFILE);
                    banList.addBan(target.getDisplayName(),"Banned by console.", null, "Console");
                }
                else {
                ...
                if (target != null) {
                    ...
                    BanList<?> banList = Bukkit.getServer().getBanList(BanList.Type.PROFILE);
                    banList.addBan(target.getDisplayName(),ChatColor.RED + stringBuilder.toString(), null, "Console");
                }
                else {
                    Bukkit.getServer().getConsoleSender().sendMessage(ChatColor.GOLD + "Player not found.");
                }
